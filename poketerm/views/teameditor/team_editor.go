...
func (m editTeamModel) View() string {
	...
	return rendering.GlobalCenter(lipgloss.JoinVertical(lipgloss.Center, lipgloss.JoinHorizontal(lipgloss.Center, selection, m.teamView.View()), "Press S to Save Team", "Press TAB to switch between new Pokemon and already created Pokemon"))
}
	...
	if m.confirming {
		...
		case tea.KeyMsg:
			if key.Matches(msg, confirm) {
				if err := teamfs.SaveTeam(global.Opt.TeamSaveLocation, m.saveNameInput.Value(), m.ctx.team); err != nil {
					m.erroring = true
					m.displayError = err
	...
	case tea.KeyMsg:
		...
		case tea.KeyEnter:
			if m.saveNameInput.Value() != "" {
				teams, _ := teamfs.LoadTeamMap(global.Opt.TeamSaveLocation)
